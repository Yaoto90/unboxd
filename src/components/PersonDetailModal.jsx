...
export default function PersonDetailModal({ personId, onClose, onSelectMovie }) {
  ...
  useEffect(() => {
    ...
    Promise.all([getPersonDetails(personId), getPersonMovieCredits(personId)])
                ...
                <div className={styles.metaRow}>
                  {person.known_for_department && (
                    <span className={styles.metaBadge}>
                      <Clapperboard size={16} /> {DEPT_MAP[person.known_for_department] || person.known_for_department}
                    </span>
                  )}
                  {person.birthday && (
