...
import { Heart, Trash2, Pencil, EyeOff, Eye, MessageCircle } from 'lucide-react';
...
export default function ReviewCard({
  ...
  const [revealedSpoiler, setRevealedSpoiler] = useState(true);
  ...
  const handleToggleLike = async (e) => {
    e.stopPropagation();
    if (!user) {
      return onOpenAuth ? onOpenAuth('signin') : alert('Please sign in to like reviews.');
    }
  ...
  const handleUserClick = (e) => {
    ...
    e.stopPropagation();
    onCloseModal?.();
    if (user && review.user_id === user.id) navigate('/profile');
    else if (username && username !== 'user') navigate(`/user/${username}`);
  };

  const hasSpoiler = Boolean(review.contains_spoilers && !revealedSpoiler);
  const replyCount = review.review_replies?.[0]?.count || 0;

  return (
    <div
      onClick={() => onSelectMovie?.(review.tmdb_movie_id, review.id)}
    ...
    >
      {showMoviePoster && review.movie_poster_path && (
        <div className={styles.posterWrap}>
          <img src={getImageUrl(review.movie_poster_path, 'w185')} alt={review.movie_title || 'Film'} className={styles.posterImg} />
        </div>
      )}

      <div className={styles.mainBody}>
        {showMoviePoster && <h4 className={styles.movieTitle}>{review.movie_title}</h4>}
        
        <div className={styles.userRow}>
          <div onClick={handleUserClick} className={styles.userInfo}>
            <div className={styles.avatarCircle}>
              {avatarUrl ? (
                <img src={avatarUrl} alt={username} className={styles.avatarImg} onError={(e) => { e.currentTarget.style.display = 'none'; }} />
              ) : (
        ...
        <div className={styles.textSection}>
          {hasSpoiler ? (
            <div onClick={(e) => { e.stopPropagation(); setRevealedSpoiler(true); }} className={styles.spoilerBanner}>
      ...
      <div className={styles.footer}>
          ...
          {!showMoviePoster && (
            <>
              ...
              <button 
                ...
                title="View replies"
                className={`${styles.likeBtn} ${styles.unliked}`}
              >
        ...
        <div className={styles.actionsRight}>
          {review.contains_spoilers && revealedSpoiler && (
            <button onClick={(e) => { e.stopPropagation(); setRevealedSpoiler(false); }} title="Hide spoilers again" className={styles.hideBtn}>
              <Eye size={13} /> <span>Hide</span>
            </button>
          )}
          ...
          {user?.id === review.user_id && (
            <>
              {onEdit && (
                <button onClick={(e) => { e.stopPropagation(); onEdit(review); }} title="Edit review" className={styles.iconBtn}>
                  <Pencil size={14} />
                </button>
              )}
              {onDelete && (
                <button onClick={(e) => { e.stopPropagation(); onDelete(review.id); }} title="Delete review" className={`${styles.iconBtn} ${styles.iconBtnDelete}`}>
                  <Trash2 size={14} />
                </button>
              )}
            </>
